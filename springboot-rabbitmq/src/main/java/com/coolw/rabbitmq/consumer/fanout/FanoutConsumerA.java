...
@Slf4j
@Component
public class FanoutConsumerA extends AbstractConsumer<String> {

    @Override
    @RabbitListener(queues = "fanout.A")
    public void process(MessageDTO<String> messageDTO, Channel channel, Message message) {
        log.info("FanoutConsumerA接收到消息,消息:{}", messageDTO);
        try {
            ack(messageDTO, channel, message);
        } catch (Exception e) {
            log.error("FanoutConsumerA消费者处理异常,messageId:{}", messageDTO.getMessageId(), e);
