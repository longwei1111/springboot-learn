...
@Slf4j
@Component
public class FanoutConsumerB extends AbstractConsumer<String> {

    @Override
    @RabbitListener(queues = "fanout.B")
    public void process(MessageDTO<String> messageDTO, Channel channel, Message message) {
        log.info("FanoutConsumerB接收到消息,消息:{}", messageDTO);
        try {
            ack(messageDTO, channel, message);
        } catch (Exception e) {
            log.error("FanoutConsumerB消费者处理异常,messageId:{}", messageDTO.getMessageId(), e);
